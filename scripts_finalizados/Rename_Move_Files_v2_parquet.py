...
for file_to_pre_processing in files_to_pre_processing: #--> Iterando sobre a lista encontrada
    ...
    file_to_pre_processing_csv = file_to_pre_processing.replace('.gpx','.parquet') #--> Ajustando o nome do arquivo para ficar com a extensão .parquet ao invés de .gpx
    info_rota_df = pd.DataFrame(info_rota) #--> Criando um dataframe a partir da lista
    info_rota_df.to_parquet(f'{PRE_PROCESSING}/{file_to_pre_processing_csv}', index=False) #--> Criando um .parquet a partir do DF criado acima e com o nome definido na variável anterior
...
################################################################################################################
files_to_datalake = [files for files in os.listdir(PRE_PROCESSING) if files.endswith(".parquet")] #--> Listando todos os arquivos da pasta pré-processamento com extensão .parquet
for name_file in files_to_datalake: #--> Iterando sobre cada item da lista
    local_path = os.path.join(PRE_PROCESSING, name_file) #--> Criando o caminho completo para o arquivo .parquet
